...
export default function CreatorAnalysis() {
  ...
  const saveNiche = async (handle) => {
    ...
    toast.success(`Niche @${handle} disimpan`);
  ...
  const chartOpt = {
    ...
    yAxis: {
      type: "category",
      data: data.creators.slice(0, 12).map((c) => c.is_organic ? "Organik" : `@${c.handle}`).reverse(),
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-5" data-testid="creator-page">
      <div>
        <div className="text-xs uppercase tracking-wider text-stone-500 mb-1">Affiliate</div>
        <h1 className="font-display text-3xl sm:text-4xl font-extrabold tracking-tight text-stone-900">
          Analisis Creator
        </h1>
        <p className="text-stone-600 mt-2">Performa kreator afiliasi diambil dari field yang terekstrak.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 pp-card p-5">
          <h3 className="font-display font-bold text-base mb-2 flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-orange-700" /> Ranking Creator
          </h3>
          {data.creators.length ? <ReactECharts option={chartOpt} style={{ height: 340 }} /> : <EmptyBlock />}
        </div>
        <div className="pp-card p-5">
          <h3 className="font-display font-bold text-base mb-2 flex items-center gap-2">
            <Tag className="w-4 h-4 text-orange-700" /> Rekap per Niche
          </h3>
      ...
      <div className="pp-table-scroll" data-testid="creator-table">
        <table className="pp-table">
          <thead>
            <tr>
              ...
              <th className="text-right">Unique Buyer</th>
            ...
            {data.creators.map((c) => {
              ...
              return (
                <tr key={key}>
                  <td>
                    {c.is_organic ? (
                      <span className="italic text-stone-500">Organik / tanpa creator</span>
                    ) : (
                  ...
                  <td>
                    {c.is_organic ? "—" : editing[c.handle] ? (
                      <input
                        ...
                        placeholder="mis. MUA / wedding"
                        autoFocus
                      />
                    ) : (
                      <span className="text-stone-700 text-xs">{c.niche || <span className="text-stone-400">— set niche —</span>}</span>
                    )}
                  </td>
                  ...
                  <td>
                    {!c.is_organic && (
                      editing[c.handle] ? (
                        <button onClick={() => saveNiche(c.handle)} className="text-xs pp-link">Simpan</button>
                      ) : (
        ...
        {data.creators.length === 0 && (
          <div className="text-center py-12 text-sm text-stone-500">Belum ada data creator.</div>
        )}
      </div>
    </div>
  );
...
function EmptyBlock() {
  return <div className="py-8 text-center text-sm text-stone-500">Belum ada data.</div>;
}
